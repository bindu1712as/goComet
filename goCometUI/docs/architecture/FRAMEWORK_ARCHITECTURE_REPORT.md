...
- [framework/fixtures/testfixtures.ts](#L16)
- [tests/search/search.spec.ts](#L5)
...
|---|---|---|
| []() | Scripts and dependency graph | npm scripts run Playwright, typecheck, and optional AI utilities |
| [playwright.config.ts](#L3) | Global Playwright behavior | Controls retries, workers, reporters, artifacts, project list |
| [Jenkinsfile](#L1) | Jenkins pipeline definition | Executes tests and publishes reports/artifacts |
| []() | Repository hygiene | Controls ignored artifacts and generated outputs |
...
#### Main file
- [framework/fixtures/testfixtures.ts](#L1)

#### Classes/Types/Methods
- Type: `MyFixtures` in [framework/fixtures/testfixtures.ts](#L7)
- Fixture extension: `base.extend<MyFixtures>` in [framework/fixtures/testfixtures.ts](#L16)
- Factory fixtures:
  - `LoginPage` in [framework/fixtures/testfixtures.ts](#L18)
  - `dashboardPage` in [framework/fixtures/testfixtures.ts](#L22)
  - `searchPage` in [framework/fixtures/testfixtures.ts](#L26)
...
#### Runtime flow
1. Test imports custom `test` from [framework/fixtures/testfixtures.ts](#L16).
...
- Core methods:
  - `usernameTextbox` at [framework/pages/LoginPage.ts](#L15)
  - `passwordTextbox` at [framework/pages/LoginPage.ts](#L25)
  - `loginButton` at [framework/pages/LoginPage.ts](#L31)
  - `navigate` at [framework/pages/LoginPage.ts](#L45)
  - `login` at [framework/pages/LoginPage.ts](#L64)
  - `verifyInvalidLogin` at [framework/pages/LoginPage.ts](#L90)
- Dependencies:
  - Playwright Page/Locator/expect
  - Logger from [framework/utils/logger.ts](#L18)
- Invoked by:
  - [tests/login/valid-login.spec.ts](#L12)
  - [tests/login/invalid-login.spec.ts](#L11)
  - [tests/search/search.spec.ts](#L17)

##### File: [framework/pages/DashboardPage.ts](#L3)
- Class: `DashboardPage`
- Purpose: verify successful post-login landing
- Core methods:
  - `dashboardHeader` at [framework/pages/DashboardPage.ts](#L7)
  - `verifyDashboardLoaded` at [framework/pages/DashboardPage.ts](#L24)
- Notable behavior:
  - Uses `.or()` locator chaining to tolerate alternate DOM structures.
- Invoked by:
  - [tests/login/valid-login.spec.ts](#L17)
  - [tests/search/search.spec.ts](#L23)

##### File: [framework/pages/searchPage.ts](#L4)
- Class: `SearchPage`
- Purpose: search menu and assert result visibility
- Core methods:
  - `searchInput` (self-healing) at [framework/pages/searchPage.ts](#L19)
  - `searchResult` at [framework/pages/searchPage.ts](#L56)
  - `search` at [framework/pages/searchPage.ts](#L62)
  - `verifyResult` at [framework/pages/searchPage.ts](#L69)
  - `searchAndVerify` at [framework/pages/searchPage.ts](#L78)
- Self-healing dependency:
  - [framework/utils/selfHealingLocator.ts](#L65)
- Invoked by:
  - [tests/search/search.spec.ts](#L25)
...
##### File: [framework/utils/env.ts](#L1)
- Purpose: static environment values (URL and credentials).
- Runtime: consumed by tests before navigation/login.

##### File: [framework/utils/logger.ts](#L18)
- Class: `Logger`
- Purpose: leveled structured logging with in-memory collection and optional file persistence.
- Core API:
  - `info` [framework/utils/logger.ts](#L55)
  - `warn` [framework/utils/logger.ts](#L59)
  - `error` [framework/utils/logger.ts](#L63)
  - `debug` [framework/utils/logger.ts](#L67)
  - `saveLogs` [framework/utils/logger.ts](#L81)
- Runtime use: Login and AI modules.

##### File: [framework/utils/selfHealingLocator.ts](#L18)
...
- Important methods:
  - `get` [framework/utils/selfHealingLocator.ts](#L65)
  - `isValid` [framework/utils/selfHealingLocator.ts](#L126)
...
- Methods:
  - `collectArtifacts` [framework/ai/failureCollector.ts](#L39)
  - `saveArtifact` [framework/ai/failureCollector.ts](#L76)

##### File: [framework/ai/failureClassifier.ts](#L18)
- Class: `FailureClassifier`
- Heuristic categorization from error text/stack/logs.
- Entry method:
  - `classify` [framework/ai/failureClassifier.ts](#L23)
...
- Methods:
  ...
  - `generateHtmlReport` [framework/ai/reportGenerator.ts](#L87)
  - `generateMarkdownReport` [framework/ai/reportGenerator.ts](#L179)

## 4. Source Utility Layer

### 4.1 framework/utils/selfHealingLocator.ts

#### Purpose
Active runtime self-healing implementation for Search test path.

#### Entry point
- `healLocator` at [framework/utils/selfHealingLocator.ts](#L65)

#### Key components
- `FallbackDescriptor` [framework/utils/selfHealingLocator.ts](#L20)
- `HealLocatorOptions` [framework/utils/selfHealingLocator.ts](#L27)
- `HealResult` [framework/utils/selfHealingLocator.ts](#L40)
- `isLocatorUsable` [framework/utils/selfHealingLocator.ts](#L154)
- `attachRecoveryReport` [framework/utils/selfHealingLocator.ts](#L182)

## 5. Test Suite and Invocation Model

### 5.1 Search Test
- File: [tests/search/search.spec.ts](#L5)
- Runtime chain:
...
### 5.2 Login Tests
- Files:
  - [tests/login/valid-login.spec.ts](#L5)
  - [tests/login/invalid-login.spec.ts](#L5)
...
flowchart TD
    ...
    G --> G1[framework/utils/selfHealingLocator.ts]
...
sequenceDiagram
    ...
    CLI->>CFG: Load [playwright.config.ts](#L3)
...
1. npm maps `test` to `playwright test` in []().
2. Playwright reads [playwright.config.ts](#L3).
3. Test files discovered under []().
4. Custom fixture model loaded from [framework/fixtures/testfixtures.ts](#L16).
5. Browser project starts (`chromium`) using [playwright.config.ts](#L33).
6. Each test executes page-object methods.
7. Search test calls self-healing utility via [framework/pages/searchPage.ts](#L19).
8. Reporters emit HTML and Allure outputs based on [playwright.config.ts](#L17).
...
- Two self-healing implementations coexist:
  - Active: [framework/utils/selfHealingLocator.ts](#L65)
  - Legacy: [framework/utils/selfHealingLocator.ts](#L65)
- AI modules are present but not globally wired through a fixture-level failure hook.
- `chromadb` exists as dependency in [](), but the current vector store is JSON keyword matching in [framework/ai/vectorStore.ts](#L82).
...
4. Externalize environment values from [framework/utils/env.ts](#L1) to secure environment variables.
